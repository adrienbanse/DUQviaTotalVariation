...
def systemDynamics(x):

    comp1 = x[0] + 0.1*x[1]
    comp2 = x[1] + 0.1*(1/3*x[0]**3 - x[0] - x[1])
...
def stateEvolution(initial_state, mean_noise, cov_noise, n_steps_ahead):
    ...
    for t in range(n_steps_ahead):
        ...
        state = systemDynamics(state) + noise
    
    return state


def monteCarloSimulationSystem(n_simulations, mean_initial_state, cov_initial_state, mean_noise, cov_noise, n_steps_ahead):
    ...
    for i in range(n_simulations):
        ...
        final_state = stateEvolution(initial_state, mean_noise, cov_noise, n_steps_ahead)
...
def propagateSignatures(signatures):
    ...
    for point in signatures:
        ...
        propagated_point = systemDynamics(point)
        gaussian_means.append(propagated_point)
    
    return gaussian_means


def sampleFromGMM(n_samples, weights, signatures, cov):
                
    sampled_points = []

    means_gmm = propagateSignatures(signatures) #See equation to see the need of this propagation (THINK: IT SEEMS TO DEPEND ON HOW WE BUILD PROPAGATION)
...
def propagateUncertaintyOneStep(regions, signatures, probas, cov_noise):
    ...
    means_gmm = propagateSignatures(signatures) #OR SHOULD IT BE JUST SIGNATURES?

    samples = sampleFromGMM(1000, probas, signatures, cov_noise)
        
    params_x0, params_x1 = defineRegionParameters(samples)


    n_signatures = math.floor(6*params_x0[1]/0.07) #proxy
    print(n_signatures)
